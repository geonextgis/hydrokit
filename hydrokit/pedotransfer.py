...
def Lambda_Maidment92(phi, clay, sand):
    ...
    Lambda = np.exp(
        ...
        + 0.0177544 * sand
        - 1.062498 * phi
        - 0.00005304 * sand * sand
        - 0.00273493 * clay * clay
        + 1.11134946 * phi * phi
        - 0.03088295 * sand * phi
...
def Residual_Water_Content_Maidment92(phi, clay, sand):
    ...
    SMR = (
        ...
        + 0.00513488 * clay
        + 0.02939286 * phi
        - 0.00015395 * clay * clay
        - 0.0010827 * sand * phi
        - 0.00018233 * clay * clay * phi * phi
        + 0.00030703 * clay * clay * phi
        - 0.0023584 * phi * phi * clay
    )
    return SMR
...
def Run_Tests():
    ...
    clay = (
        np.array(
            [88.0, 80.0, 65.0, 40.0, 20.0, 10.0, 60.0, 30.0, 10.0, 10.0, 50.0, 25.0]
        )
        / 100
    )
    sand = (
        np.array([5.0, 5.0, 10.0, 20.0, 15.0, 5.0, 25.0, 35.0, 35.0, 45.0, 40.0, 50.0])
        / 100
    )
    om = 2.5
    ksat = np.array(
        [108.0, 96.7, 50.3, 15.5, 16.1, 22.0, 11.3, 4.3, 5.7, 3.7, 1.4, 1.1]
    )
    thetas = (
        np.array(
            [46.0, 46.0, 45.0, 46.0, 48.0, 48.0, 43.0, 48.0, 51.0, 52.0, 44.0, 50.0]
        )
        / 100
    )
    theta33 = (
        np.array(
            [10.0, 12.0, 18.0, 28.0, 31.0, 30.0, 27.0, 36.0, 38.0, 41.0, 36.0, 42.0]
        )
        / 100
    )
    theta1500 = (
        np.array([5.0, 5.0, 8.0, 14.0, 11.0, 6.0, 17.0, 22.0, 22.0, 27.0, 25.0, 30.0])
        / 100
    )
